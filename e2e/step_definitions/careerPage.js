...
defineSupportCode(({Given, When, Then, setDefaultTimeout}) => {
    ...
    When(/^the (country|city) (.+) is selected$/, (type, name) => {
        if (type === "country"){
            return careerPage.selectCountryFromDropdown(name);
        }
        return careerPage.selectCity(name);
        // (type === "country")?return careerPage.selectCountryFromDropdown(name):return careerPage.selectCity(name);
    });
    ...
    Then(/(.+) should be selected in the location filter box/, city => {
        return expect(careerPage.selectedLocation).to.eventually.equal(city);
    });
